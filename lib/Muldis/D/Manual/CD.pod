...
This document is Muldis::D::Manual::CD version 0.9.0.
...
=head2 PTMD_STD - insert_db
    ...
    recipe insert_records (&db ::= fed.data.db, src ::= nlx.data) {
        ...
        let max_used_artist_id ::= is_empty(db.artists) ?? 0
            !! max( Set_from_attr( db.artists, name => Name:artist_id ) )
        let max_used_cd_id ::= is_empty(db.cds) ?? 0
            !! max( Set_from_attr( db.cds, name => Name:cd_id ) )
        let max_used_track_id ::= is_empty(db.tracks) ?? 0
        ...
        let src_artists_with_ids ::= rank_by_attr_names( src.artists,
            name => Name:artist_id,
            order_by => [ %:{ name => Name:artist_name,
                is_reverse_order => False } ],
            first_rank => max_used_artist_id++
        )
        let src_cds_with_ids ::= rank_by_attr_names( src.cds,
            name => Name:cd_id,
            order_by => [ %:{ name => Name:cd_title,
                is_reverse_order => False } ],
            first_rank => max_used_cd_id++
        )
        let src_tracks_with_ids ::= rank_by_attr_names( src.tracks,
            name => Name:track_id,
        ...
        let new_artists ::= src_artists_with_ids
        let new_cds ::= src_cds_with_ids compose new_artists
        let new_tracks ::= src_tracks_with_ids compose new_cds{!artist_id}
...
=head2 HDMD_Perl6_STD - the_database
    ...
    [
        [ 'Muldis_D', 'http://muldis.com', '0.148.0', 'HDMD_Perl6_STD', {
            catalog_abstraction_level => 'rtn_inv_alt_syn',
            op_char_repertoire => 'basic'
        } ],
    ...
    [ 'relation-type', 'artists', [
        :tuple-type<nlx.lib.T>,
        :with[ 'tuple-type', 'T', [
            [ 'attr', 'artist_id'  , 'Int'  ],
            [ 'attr', 'artist_name', 'Text' ],
        ] ],
        :constraint<nlx.lib.pk>,
        :with[ 'primary-key', 'pk', 'artist_id' ],
    ...
    [ 'relation-type', 'tracks', [
        ...
        :constraint<nlx.lib.pk>,
        :with[ 'primary-key', 'pk', 'track_id' ],
        :constraint<nlx.lib.sk_track_title>,
        :with[ 'key-constraint', 'sk_track_title', 'track_title' ],
    ] ],

    [ 'subset-constraint', 'sc_cd_has_tracks', {
        parent      => 'cds',
        using-key   => 'nlx.lib.cds.pk',
        child       => 'tracks',
    ...
    [ 'recipe', 'get_tracks_by_artist',
        ...
        [ 'i-op', ':=', [ :d<track_titles>,
            [ 'curried-func', 'Array_from_attr', [
                [ 'i-op', 'matching', [ :acc<db.tracks>,
                    [ 'i-op', 'matching', [ :acc<db.cds>,
                        [ 'i-op', 'matching', [ :acc<db.artists>,
                            '@' => Set.new(
...
=head2 HDMD_Perl6_STD - insert_db
    ...
    [ 'procedure', 'main', {} => [
        # Connect to the_database in read-write mode. #
        # The "..." has likes of filename, server name, user, pass, etc. #
        [ 'proc-invo', 'create_depot_mount', { name => :Name<db>,
        ...
        # This might fail if records with same titles/etc already there. #
        [ 'try-catch',
            :proc-invo<nlx.lib.insert_records>,
            :proc-invo<nlx.lib.insert_failed>
        ],

        # Disconnect the_database now that we're done with it. #
        [ 'proc-invo', 'drop_depot_mount', { name => :Name<db> } ],
    ] ],

    [ 'procedure', 'insert_failed', { err => 'Exception' } => [
        [ 'proc-invo', 'write_Text_line',
...
=head2 HDMD_Perl6_STD - test_db
    ...
    [ 'transaction', 'get_records', {} => [
        [ 'proc-invo', 'nlx.lib.get_tracks_by_cd',
            { cd_title => 'Bad' } ],
        [ 'proc-invo', 'nlx.lib.get_tracks_by_artist',
            { artist_name => 'Michael Jackson' } ],

        [ 'proc-invo', 'nlx.lib.get_cd_by_track',
            { track_title => 'Stan' } ],
        [ 'proc-invo', 'nlx.lib.get_cds_by_artist',
            { artist_name => 'Michael Jackson' } ],

        [ 'proc-invo', 'nlx.lib.get_artist_by_track',
            { track_title => 'Dirty Diana' } ],
        [ 'proc-invo', 'nlx.lib.get_artist_by_cd',
    ...
    [ 'procedure', 'disp_list', { h1 => 'Text', h2 => 'Text',
            list => 'array_of.Text' } => [
        [ 'var', 'eol', 'Text' ],
        [ 'proc-invo', 'end_of_line_Text', [ ['&',:d<eol>] ] ],
        [ 'proc-invo', 'write_Text', [ [ 'i-op', '~', [
            :d<h1>, '(', :d<h2>, '):',
            :d<eol>,
    ...
    [ 'procedure', 'get_tracks_by_artist', { artist_name => 'Text' } => [
        [ 'var', 'track_titles', 'array_of.Text' ],
        [ 'proc-invo', 'fed.lib.db.get_tracks_by_artist',
            { track_titles => ['&',:d<track_titles>],
            artist_name => :d<artist_name> } ],
        [ 'proc-invo', 'nlx.lib.disp_list',
...
=head2 HDMD_Perl5_STD - the_database
    ...
    [
        [ 'Muldis_D', 'http://muldis.com', '0.148.0', 'HDMD_Perl5_STD', {
            catalog_abstraction_level => 'rtn_inv_alt_syn',
            op_char_repertoire => 'basic'
        } ],
