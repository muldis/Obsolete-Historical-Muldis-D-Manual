...
standard dialects of Muldis D.  But only the PTMD_STD dialect has the
complete example, because that dialect is the most terse and readable; the
HDMD_Perl6_STD and HDMD_Perl5_STD dialects have a representative subset
only, such that one should be able to infer the missing portions easily
enough through comparison with the complete PTMD_STD example; but they
might also be made complete later if the rationale exists.
...
=head2 HDMD_Perl6_STD - the_database
    ...
    [
        [ 'Muldis_D', 'http://muldis.com', '0.129.0', 'HDMD_Perl6_STD', {
            catalog_abstraction_level => 'rtn_inv_alt_syn',
            op_char_repertoire => 'basic'
        } ],
    ...
    #######################################################################
            ...
            :depot-data[ 'Database', {
                artists => :Relation(Set.new(<artist_id artist_name>)),
                cds     => :Relation(Set.new(<cd_id artist_id cd_title>)),
                tracks  => :Relation(Set.new(<track_id cd_id track_title>))
            } ] # depot-data #
        ]
    ]
...
=head2 HDMD_Perl6_STD - insert_db
    ...
    #######################################################################
            ...
            :depot-data[ 'Database', {
                artists => :Relation( <artist_name> => Set.new(
                    [ 'Michael Jackson' ],
                    [ 'Eminem'          ]
                ) ),
                cds => :Relation( <cd_title artist_name> => Set.new(
                    [ 'Thriller'               , 'Michael Jackson' ],
                    [ 'Bad'                    , 'Michael Jackson' ],
                    [ 'The Marshall Mathers LP', 'Eminem'          ]
                ) ),
                tracks => :Relation( <track_title cd_title> => Set.new(
                    [ 'Beat It'        , 'Thriller'                ],
                    [ 'Billie Jean'    , 'Thriller'                ],
                    [ 'Dirty Diana'    , 'Bad'                     ],
                    [ 'Smooth Criminal', 'Bad'                     ],
...
=head2 HDMD_Perl5_STD - the_database
    ...
    [
        [ 'Muldis_D', 'http://muldis.com', '0.129.0', 'HDMD_Perl5_STD', {
            catalog_abstraction_level => 'rtn_inv_alt_syn',
            op_char_repertoire => 'basic'
        } ],
